...
@php
    $classes = Flux::classes()
        ->add('flex whitespace-nowrap flex-1 justify-center items-center gap-2 cursor-pointer')
        ->add('rounded-md data-checked:shadow-xs')
        ->add(
            'text-sm font-medium text-zinc-600 hover:text-zinc-800 dark:hover:text-white dark:text-white/70 data-checked:text-zinc-100 dark:data-checked:text-white',
        )
        ->add('data-checked:bg-primary-500 dark:data-checked:bg-primary-950')
        ->add(
    ...
    $iconClasses = Flux::classes(
        'text-zinc-500 dark:text-zinc-400 [ui-radio[data-checked]_&]:text-zinc-100 dark:[ui-radio[data-checked]_&]:text-white',
    )
