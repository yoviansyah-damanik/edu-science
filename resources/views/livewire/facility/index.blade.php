<div class="space-y-6">
    <flux:breadcrumbs>
        ...
        <flux:breadcrumbs.item>{{ __('Facilities') . '/' . __('Infrastructures') }} </flux:breadcrumbs.item>
    </flux:breadcrumbs>
    ...
    <div class="" data-area="page-title">
        ...
        <flux:text>
            {{ __('Manage your facilities and infrastructures.') }}
        </flux:text>
    </div>

    <div class="" data-area="filter">
        <div class="flex gap-4 flex-col lg:flex-row">
            <flux:button variant="primary" icon="plus" wire:click="$dispatch('setCreateFacility')">
                {{ __('Add :1', ['1' => __('Facility/Infrastructure')]) }}
            </flux:button>
            <flux:input.group>
        ...
        <table
            ...
            <thead
                ...
                <tr
                    ...
                    <th>{{ '#' }}</th>
                    <th>{{ __('Name of :1', ['1' => __('Facility/Infrastructure')]) }}</th>
                    <th></th>
                </tr>
            </thead>
            <tbody
                        ...
                        <td>
                            <div class="flex gap-3 justify-end">
                                <flux:button icon="pencil-line" size="sm"
                                    wire:click="$dispatch('setEditFacility',{ facility: '{{ $item->id }}' })">
                                    {{ __('Edit') }}
                                </flux:button>
                                <flux:button variant="danger" icon="trash" size="sm"
                                    wire:click="$dispatch('setDeleteFacility',{ facility: '{{ $item->id }}' })">
                                    {{ __('Delete') }}
                                </flux:button>
                            </div>
                        </td>
    ...
    <div>
        <flux:modal name="delete-facility-modal" class="w-full max-w-xl">
            <livewire:facility.delete lazy />
        </flux:modal>
        <flux:modal name="edit-facility-modal" class="w-full max-w-xl">
            <livewire:facility.edit lazy />
        </flux:modal>
        <flux:modal name="create-facility-modal" class="w-full max-w-xl">
            <livewire:facility.create lazy />
        </flux:modal>
    </div>
</div>
