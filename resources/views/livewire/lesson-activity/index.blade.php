<div class="space-y-6">
    <flux:breadcrumbs>
        ...
        <flux:breadcrumbs.item>{{ __('Lesson Activities') }} </flux:breadcrumbs.item>
    </flux:breadcrumbs>
    ...
    <div class="" data-area="page-title">
        ...
        <flux:text>
            {{ __('Manage your lesson activities.') . ' ' . __('You can add lesson activities to the lesson material menu.') }}
                ...
                @forelse ($items as $item)
                    <tr class="[&>*]:py-3 [&>*]:px-4 [&>*:first-child]:ps-0 [&>*:last-child]:pe-0 [&>*]:text-wrap">
                        ...
                        <td>
                            ...
                            <flux:link
                                :href="route('lesson-activities.show', ['lessonPlan' => $lessonPlan, 'lessonActivity' =>
                                    $item
                                ])"
                                wire:navigate>
                                {{ $item->material->title }}
                        ...
                        <td class="text-end">
                            <flux:dropdown position="left">
                                ...
                                <flux:menu>
                                    <flux:menu.item
                                        :href="route('lesson-activities.show', ['lessonPlan' => $lessonPlan,
                                            'lessonActivity' => $item
                                        ])"
                                        wire:navigate icon="eye">{{ __('Show') }}</flux:menu.item>
                                    <flux:menu.item
                                        :href="route('lesson-activities.edit', ['lessonPlan' => $lessonPlan,
                                            'lessonActivity' => $item
                                        ])"
                                        wire:navigate icon="pencil-line">{{ __('Edit') }}</flux:menu.item>
                                    <flux:menu.separator />
                                    <flux:menu.item variant="danger" icon="trash"
                                        wire:click="$dispatch('setDeleteLessonActivity',{ lessonActivity: '{{ $item->id }}', isRefresh: false })">
                                        {{ __('Delete') }}
                                    </flux:menu.item>
                                </flux:menu>
                            </flux:dropdown>
